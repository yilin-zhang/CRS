    ...
    for root, _, files in os.walk(dirname):
        for filename in files:
            ...
            if os.path.splitext(filename)[1] == '.txt':
                ...
                dir_chords.append(parse_file(filename))
    return dir_chords
...
def parse_file(filename):
    ...
    with open(filename, 'r') as lines:
        ...
        for idx, line in enumerate(lines):
            ...
            if is_section_start_point(line) and idx > 6:
                chord_sequences.append(extract_chords(section))
...
def is_section_start_point(line):
    '''If the line is the start of a new section
    Arg:
    - line: a line of text
    Return:
...
if __name__ == '__main__':
    chords = parse_directory('McGill-Billboard')
    print(chords)